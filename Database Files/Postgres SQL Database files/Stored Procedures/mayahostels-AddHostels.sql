...
BEGIN
    ...
    IF v_number_of_bedspaces_per_room <= 0 THEN
        RAISE EXCEPTION 'Number of bedspaces must be greater than 0';
    END IF;

    IF v_hostel_capacity <= 0 THEN
        RAISE EXCEPTION 'Please provide a valid hostel capacity';
    END IF;

    IF v_hostel_accommodation_price_per_semester <= 0 THEN
        RAISE EXCEPTION 'Please provide a valid hostel accommodation price per semester. It must be greater than 0.';
    END IF;
